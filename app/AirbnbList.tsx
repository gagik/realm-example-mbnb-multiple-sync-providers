...
type SearchFunction = ({
  ...
  pageNumber: number;
  pageSize: number;
}) => Promise<{ error: string; result: ListingsAndReview[] }>;
...
export const AirbnbList = () => {
  ...
  const user = useUser<
    { searchListings: SearchFunction },
    Record<string, unknown>,
    DefaultUserProfileData
  >();

  // Check the local cache for search results
  const cache = useLocalQuery(SearchCache, (col) =>
    col.filtered("searchTerm == $0", searchTerm.toLowerCase())
  );
